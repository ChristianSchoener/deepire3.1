...
import inf_common_original as IC
...
if __name__ == "__main__":
  ...
  prob_data_list = torch.load("raw_log_data_original.pt", weights_only=False)

  thax_sign,sine_sign,deriv_arits,thax_to_str = torch.load("data_sign_original.pt", weights_only=False)
  ...
  torch.save((thax_sign,sine_sign,deriv_arits,thax_to_str), "{}/data_sign.pt".format("."))
  print(f"Done; data_sign updated (and saved to {"."})")
  ...
  if True:
    ...
    for i, ((probname,probweight),(init,deriv,pars,pos_vals,neg_vals,tot_pos,tot_neg)) in enumerate(prob_data_list):
      ...
      print("name", probname, "tot_pos / tot_neg", tot_pos, tot_neg, flush=True)
  ...
  if SAVE_PIECES:
    # save just names:
    filename = "{}/training_index.pt".format(".")
    print("Saving training part to",filename)
    torch.save(prob_data_list[:spl], filename)
    filename = "{}/validation_index.pt".format(".")
    print("Saving validation part to",filename)
    torch.save(prob_data_list[spl:], filename)
  else:
    filename = "{}/training_data.pt".format(".")
    print("Saving training part to",filename)
    torch.save(prob_data_list[:spl], filename)
    filename = "{}/validation_data.pt".format(".")
    print("Saving validation part to",filename)
    torch.save(prob_data_list[spl:], filename)
