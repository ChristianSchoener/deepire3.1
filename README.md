...
To further improve on the model, I would analyse the number of nodes on each level, and make the machinery more complex on the most crowded levels: For each of them introduce multiple designated layers for every inference rule, and evaluation, as well. 
