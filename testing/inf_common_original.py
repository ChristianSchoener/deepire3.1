...
class LearningModel(torch.nn.Module):
  ...
  def contribute(self, id: int, embed : Tensor):
    ...
    pos = self.pos_vals.get(id, 0.0)
    neg = self.neg_vals.get(id, 0.0)
    if val[0].item() >= 0.0:
      self.posOK += pos
    else:
      self.negOK += neg
...
class EvalMultiModel(torch.nn.Module):
  def __init__(self,models,
    ...
    pos_weight = HP.GLOBAL_TOT_POS_TOT_NEG_RATIO if not HP.GLOBAL_TOT_POS_TOT_NEG_RATIO == 1.0 else (HP.POS_WEIGHT_EXTRA*tot_neg/tot_pos if tot_pos > 0.0 else 1.0)
  ...
  def forward(self):
    ...
    # print([param for param in self.deriv_mlps["40"].parameters()])
    for id, (thax,sine) in self.init:
      # print("init",id)
...
def compress_prob_data(some_probs):
  ...
  for (probname,probweight), (init,deriv,pars,pos_vals,neg_vals,tot_pos,tot_neg) in some_probs:
    ...
    for old_id, features in init:
      ...
      if abskey not in abs2new_init:
        new_id = id_cnt
        id_cnt += 1

        abs2new_init[abskey] = new_id
